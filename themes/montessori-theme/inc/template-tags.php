 ...
 */
  ...
  */
 function montessori_comment_count() {
 ...
 function montessori_entry_footer() {
 	...
 	if ( 'post' === get_post_type() ) {
 		...
 		if ( $categories_list && montessori_categorized_blog() ) {
  ...
  * @return bool
  */
 function montessori_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'montessori_categories' ) ) ) {
 		...
 		set_transient( 'montessori_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so montessori_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so montessori_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in montessori_categorized_blog.
  */
 function montessori_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'montessori_categories' );
 }
 add_action( 'edit_category', 'montessori_category_transient_flusher' );
 add_action( 'save_post',     'montessori_category_transient_flusher' );
  ...
  */
 function montessori_comment_list( $comment, $args, $depth ) {
  ...
  */
 function montessori_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
